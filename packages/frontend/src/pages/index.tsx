...
export default function Home({ article }) {
  ...
  return (
    <div>
      <Head></Head>
      <Header />
      <section className={`${s.sKeyVisual} w-full relative`}>
        <div className={`ps-fitParent z-10 flex flex-col justify-center`}>
          <h1 className={`${s.siteName} ps-font-en text-center font-bold text-off-white`}>Masa-Endo</h1>
          <p className={`ps-font-en text-center font-medium opacity-80 text-gray`}>Develop, Software Engineering, EM, Tech</p>
        </div>
        <div className={`${s.bg} ps-fitParent z-0`}></div>
      </section>
      <section className={`${s.sArticle} py-28`}>
        <div className="sInner">
          <ul className={`flex flex-wrap relative`}>
           {article.map(article => (
            <li className={`${s.articleItem} relative`} key={article.id}>
              <Link href={`article/${article.id}`}>
                <a>
                <div className={`${s.thumb} rounded`} style={{backgroundImage: 'url(' + `${article.thumb.url}` + ')'}}></div>
                <div className={s.contents}>
                  <h2 className={`${s.ttl} font-bold tracking-wide text-off-white`}>{article.title}</h2>
                  <p className={`${s.description} font-medium opacity-70 text-gray`}>{article.description}</p>
                </div>
                </a>
              </Link>
              <div className={`flex mt-5`}>
                    <p className={`${s.createDate} ps-font-en font-regular flex items-center opacity-70 text-gray`}>
                    ...
                    <p className={`${s.updateDate} ps-font-en font-regular flex items-center opacity-70 ml-3 text-gray`}>
                    <svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
