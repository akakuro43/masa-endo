...
export default function Home({ article }) {
  ...
  return (
    <div>
      <Head></Head>
      <Header />
      <section className={`${s.pKeyVisual} w-full relative`}>
        <div className={`u-fitParent z-10 flex flex-col justify-center`}>
          <h1 className={`${s.siteName} p-font-en text-center font-bold text-off-white`}>Masa-Endo</h1>
          <p className={`p-font-en text-center font-medium opacity-80 text-gray`}>Develop, Software Engineering, EM, Tech</p>
        </div>
        <div className={`${s.bg} u-fitParent z-0`}></div>
      </section>
      <section className={`${s.pArticle} py-28`}>
           ...
           {article.map(article => (
            <li className={`${s.articleItem} relative`} key={article.id}>
              ...
              <div className={`flex mt-5`}>
                    <p className={`${s.createDate} p-font-en font-regular flex items-center opacity-70 text-gray`}>
                    ...
                    <p className={`${s.updateDate} p-font-en font-regular flex items-center opacity-70 ml-3 text-gray`}>
                    <svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
